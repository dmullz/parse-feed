...
def parse_feed(_nlu_url,_nlu_api_key,_classify_id,_financial_classify_id, _todays_date_pretty, _todays_date_struct, _already_ingested, _use_sql, translate_url, translate_apikey, inputs, _feed_list=[]):
	...
	for feed in _feed_list:
		...
		if feed['feed_name'] == "The Knot":
			agent = None
		try:
			data = feedparser.parse(feed_url, agent=agent)
			if data.status == 403:
				error_parsing = 403
			...
			if filter_by_title(file_name, True):
				...
				if "The New York Times" in feed['publisher']:
					if "ncr_firstparagraph" in item:
						...
						print("*** " + env + " SKIPPING EMPTY NYT ARTICLE: ", article_title, " FEED:", feed['feed_url'])
						continue
				if "Dow Jones" in feed['publisher'] or "Arizent" in feed['publisher'] or "Middle Market Information" in feed['publisher']:
...
def main(_param_dictionary):
	...
	print("**** " + env + " **** DONE PARSING FEED: " + feed_item['feed_name'] + " ARTICLES TO D/L: " + str(len(parsed_feed)) + " WITH ERROR PARSING: " + str(error_parsing))
	return {
		"headers": {
			"Content-Type": "application/json",
		},
